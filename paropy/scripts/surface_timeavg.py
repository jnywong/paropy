...
if not os.path.exists('{}/surface_timeavg'.format(directory)):
    ...
    # container = np.load('{}/surface_timeavg'.format(directory))
    # data = [container[key] for key in container]
    # theta, phi, Vt, Vp, Br, dtBr = data[0], data[1], data[2], data[3],data[4], data[5]
...
c = ax.contourf(X, Y, Z, cmap='PuOr_r') # levels, transform=ccrs.PlateCarree(), cmap='PuOr_r',
                #  extend='both')
